...
use SIP\ResourceBundle\Entity\Media\DisignerHasMedia;
use Genemu\Bundle\FormBundle\Gd\File\Image;

class DesignerAdmin extends BaseTagAdmin
{
    /**
        ...
        $formMapper
            ->tab('General')
                ...
                ->with('Addition', array('class' => 'col-md-6'))
                    ...
                    ->add('instagram', null, array('label' => 'sip_designer_instagram'))
                    ->add('pinterest', null, array('label' => 'sip_designer_pinterest'))
                ...
                ->with('Settings')
                    ...
                    ->add('tags', 'genemu_jqueryselect2_entity', array('class' => 'SIP\ResourceBundle\Entity\Tag\Tag',
                        'label'    => 'sip_designer_tags',
                        'property' => 'name',
                        'multiple' => true,
                        'required' => false))
                    ->add('brands', 'genemu_jqueryselect2_entity', array('class' => 'SIP\ResourceBundle\Entity\Brand',
                        'label'    => 'sip_designer_brands',
                        'property' => 'h1',
                        'multiple' => true,
                        'required' => false))
                    ->end()
                    ->with('Gallery')
                        ...
                        ->add('gallery', 'sonata_type_collection', array(
                                ...
                                'inline'       => 'table',
                                'sortable'     => 'position',
                                'admin_code'   => 'sip.resource.designerhasmedia.admin',
                            )
                        )
                    ->end()
                ->end()
    ...
    /**
     * @param Designer $object
     * @return mixed
     */
    public function generateSiteUrl(Designer $object)
    {
        return $this->container->get('router')->generate('sip_resource_designer', array('slug' => $object->getSlug()));
    }
}